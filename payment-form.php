...
<style>
.payment-page{--pay-radius:18px}
.payment-head{display:flex;justify-content:space-between;align-items:flex-start;gap:14px;margin-bottom:18px}
.payment-head h1{margin:8px 0 4px;font-weight:850}
.payment-card{background:var(--card-bg);border:1px solid var(--border-soft);border-radius:var(--pay-radius);box-shadow:var(--shadow)}
.payment-card-head{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:18px 20px;border-bottom:1px solid var(--border-soft)}
.payment-card-head h2{margin:0;font-size:16px;font-weight:850}
.payment-card-body{padding:20px}
.client-summary{display:none;margin-top:15px;padding:15px;border-left:4px solid var(--brand);border-radius:14px;background:var(--body-bg)}
.client-summary.show{display:block}
.stat-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}
.stat-box{padding:12px;border:1px solid var(--border-soft);border-radius:12px;background:var(--card-bg)}
.stat-box small{display:block;color:var(--text-muted);font-weight:700}
.stat-box strong{display:block;margin-top:4px;font-size:17px}
.allocation-row{display:grid;grid-template-columns:36px minmax(160px,1.6fr) 130px 130px 150px;gap:12px;align-items:center;padding:12px 0;border-bottom:1px solid var(--border-soft)}
.allocation-row:last-child{border-bottom:0}
.invoice-meta small{display:block;color:var(--text-muted)}
.amount-input{text-align:right;font-weight:750}
.payment-summary{max-width:430px;margin-left:auto}
.payment-summary-row{display:flex;justify-content:space-between;gap:18px;padding:10px 0;border-bottom:1px solid var(--border-soft)}
.payment-summary-row.total{font-size:18px;font-weight:900;border-bottom:0}
.payment-actions{position:sticky;bottom:10px;z-index:20;display:flex;justify-content:flex-end;gap:10px;padding:12px;margin-top:16px;border:1px solid var(--border-soft);border-radius:16px;background:color-mix(in srgb,var(--card-bg) 94%,transparent);backdrop-filter:blur(12px);box-shadow:var(--shadow)}
.select2-container{width:100%!important}
.select2-container .select2-selection--single{height:46px;border:1px solid var(--border-soft);border-radius:12px;background:var(--card-bg)}
.select2-container--default .select2-selection--single .select2-selection__rendered{line-height:44px;padding-left:13px;color:var(--text-main)}
.select2-container--default .select2-selection--single .select2-selection__arrow{height:44px}
.empty-box{text-align:center;padding:28px;border:1px dashed var(--border-soft);border-radius:14px;color:var(--text-muted)}
@media(max-width:991.98px){.allocation-row{grid-template-columns:36px 1fr 1fr}.allocation-row .invoice-meta{grid-column:2/-1}.stat-grid{grid-template-columns:1fr 1fr}}
@media(max-width:767.98px){.payment-head{flex-direction:column}.payment-head .btn{width:100%}.payment-card-body,.payment-card-head{padding:15px}.stat-grid{grid-template-columns:1fr}.allocation-row{grid-template-columns:32px 1fr}.allocation-row>*{grid-column:2}.allocation-row>.allocation-check{grid-column:1;grid-row:1}.payment-actions{flex-direction:column-reverse}.payment-actions .btn{width:100%}}
</style>
...
<div class="payment-page">
    <div class="payment-head">
        <div>
            ...
            <p class="mb-0 text-muted">Record a hospital payment and allocate it against unpaid invoices.</p>
        </div>
    ...
    <form id="paymentForm" novalidate>
        ...
        <section class="payment-card mb-3">
            ...
            <div class="payment-card-body">
                <div class="row g-3">
                    <div class="col-lg-5">
                        <label class="form-label fw-semibold" for="clientId">Hospital <span class="text-danger">*</span></label>
                        <select class="form-select" name="client_id" id="clientId" required>
                            <option value="">Search hospital...</option>
                            <?php foreach ($clients as $client): ?>
                                <option value="<?= (int)$client['id'] ?>"
                                        data-code="<?= e($client['client_code'] ?? '') ?>"
                    ...
                    <div class="col-md-4 col-lg-2">
                        <label class="form-label fw-semibold" for="paymentDate">Payment Date <span class="text-danger">*</span></label>
                        <input class="form-control" type="date" name="payment_date" id="paymentDate" value="<?= e($today) ?>" required>
                    </div>

                    <div class="col-md-4 col-lg-2">
                        <label class="form-label fw-semibold" for="amount">Amount <span class="text-danger">*</span></label>
                        <input class="form-control fw-bold text-end" type="number" name="amount" id="amount" min="0.01" step="0.01" placeholder="0.00" required>
                    </div>

                    <div class="col-md-4 col-lg-3">
                        <label class="form-label fw-semibold" for="paymentModeId">Payment Mode <span class="text-danger">*</span></label>
                    ...
                    <div class="col-md-4">
                        <label class="form-label fw-semibold" for="transactionReference">Transaction / Cheque Reference</label>
                        <input class="form-control" name="transaction_reference" id="transactionReference" maxlength="150" placeholder="UPI ID, UTR, cheque no., etc.">
                    ...
                    <div class="col-md-4">
                        ...
                        <input class="form-control" name="payer_name" id="payerName" maxlength="200" placeholder="Hospital or person name">
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select" name="financial_year_id" id="financialYearId">
                            ...
                            <?php foreach ($financialYears as $fy): ?>
                                <option value="<?= (int)$fy['id'] ?>" <?= $selectedFinancialYearId === (int)$fy['id'] ? 'selected' : '' ?>>
                    ...
                    <div class="col-md-8">
                        ...
                        <input class="form-control" name="notes" id="notes" maxlength="1000" placeholder="Optional payment note">
                    </div>
                </div>
                ...
                <div class="client-summary" id="clientSummary">
                    ...
                    <div class="stat-grid">
                        <div class="stat-box"><small>Total Outstanding</small><strong id="summaryOutstanding">₹0.00</strong></div>
                        <div class="stat-box"><small>Open Invoices</small><strong id="summaryInvoiceCount">0</strong></div>
        ...
        <section class="payment-card">
            <div class="payment-card-body">
                <div class="payment-summary">
                    <div class="payment-summary-row"><span>Payment Amount</span><strong id="paymentAmountText">₹0.00</strong></div>
                    <div class="payment-summary-row"><span>Allocated Amount</span><strong id="allocatedAmountText">₹0.00</strong></div>
                    <div class="payment-summary-row total"><span>Unallocated Amount</span><strong id="unallocatedAmountText">₹0.00</strong></div>
                </div>
            </div>
        </section>
        ...
        <div class="payment-actions">
            ...
            <button class="btn btn-brand" type="submit" id="savePaymentButton">
                <i data-lucide="badge-check"></i> Save Payment
            </button>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $('#clientId').select2({placeholder:'Search hospital...',allowClear:true,width:'100%'});
    ...
    async function loadInvoices() {
        ...
        if (!clientId) {
            ...
            allocationList.innerHTML = '<div class="empty-box">Select a hospital to load its unpaid invoices.</div>';
            calculateAllocation();
            return;
        }

        allocationList.innerHTML = '<div class="empty-box"><span class="spinner-border spinner-border-sm me-2"></span>Loading invoices...</div>';
        ...
        try {
            ...
            document.getElementById('summaryOutstanding').textContent = money(result.data.total_outstanding);
            document.getElementById('summaryInvoiceCount').textContent = String(result.data.invoices.length);
            summary.classList.add('show');

            if (!result.data.invoices.length) {
                allocationList.innerHTML = '<div class="empty-box">No unpaid issued invoices found for this hospital.</div>';
                calculateAllocation();
                return;
            }
            ...
            allocationList.innerHTML = result.data.invoices.map(invoice => `
                <div class="allocation-row" data-invoice-id="${invoice.id}" data-balance="${invoice.balance_amount}">
                    ...
                    <div>
                        <label class="small text-muted">Allocate</label>
            ...
            allocationList.innerHTML = `<div class="empty-box text-danger">${escapeHtml(error.message)}</div>`;
        }
    }
    ...
    function autoAllocate() {
        let remaining = Math.max(0, Number(amountInput.value || 0));
    ...
    function calculateAllocation() {
        const paymentAmount = Math.max(0, Number(amountInput.value || 0));
        ...
        document.getElementById('allocationsJson').value = JSON.stringify(collectAllocations());
    }
    ...
    allocationMethod.addEventListener('change', () => {
        ...
        if (allocationMethod.value === 'unallocated') {
            allocationList.querySelectorAll('.invoice-check').forEach(check => { check.checked = false; });
            allocationList.querySelectorAll('.allocation-amount').forEach(input => { input.value = '0'; input.disabled = true; });
            calculateAllocation();
    ...
    form.addEventListener('submit', async event => {
        ...
        if (paymentAmount <= 0) return AppToast.show('warning', 'Enter a valid payment amount.');
        if (!modeSelect.value) return AppToast.show('warning', 'Select a payment mode.');
        if (referenceInput.required && !referenceInput.value.trim()) return AppToast.show('warning', 'Enter the transaction reference.');
        if (allocated > paymentAmount + 0.001) return AppToast.show('warning', 'Allocated amount cannot exceed payment amount.');
        ...
        try {
            ...
            catch { throw new Error(raw.replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim() || 'Invalid server response.'); }
            AppToast.show(result.success ? 'success' : 'error', result.message);
            if (result.success) {
                setTimeout(() => window.location.href = '<?= e(app_url('payments.php?created=')) ?>' + result.data.payment_id, 450);
            }
        } catch (error) {
