...
if ($action === 'reverse_payment') {
    ...
    if ($paymentId <= 0 || $reason === '') {
        json_response(false, 'Payment and reversal reason are required.', [], 422);
    }
    ...
    try {
        ...
        if ($payment['payment_status'] === 'reversed') throw new RuntimeException('Payment is already reversed.');
        ...
        $pdo->commit();
        json_response(true, 'Payment reversed successfully.');
    } catch (Throwable $e) {
        if ($pdo->inTransaction()) $pdo->rollBack();
        error_log('[PAYMENT REVERSE] ' . $e->getMessage());
        json_response(false, $e->getMessage(), [], 422);
    }
}
